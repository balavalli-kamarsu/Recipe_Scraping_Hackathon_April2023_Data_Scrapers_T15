		...
		for (int k = 0; k < pageBeginsWithList.size(); k++) {
			...
			if (0 != lastPage) {
				for (int j = 1; j <= lastPage; j++) {
					...
					/*
					 * for (int j = 1; j <= 22; j++) { int pageindex = j;
					 * driver.navigate().to("https://www.tarladalal.com/RecipeAtoZ.aspx?pageindex="
					 * + j); List<WebElement> recipeCardElements =
					 * driver.findElements(By.xpath("//div[@class='rcc_recipecard']")); List<String>
					 * recipeUrls = new ArrayList<>(); Map<String, String> recipeIdUrls = new
					 * HashMap<>();
					 */
